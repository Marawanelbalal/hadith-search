...
const HomePage = () => {
  ...
  return (
    <main className="flex-grow flex flex-col items-center justify-center px-margin-mobile md:px-margin-desktop py-16 w-full max-w-container-max-width mx-auto relative">
      {/* Background gradient */}
      <div className="absolute inset-0 pointer-events-none bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-surface-container-low via-background to-background opacity-50 -z-10" />
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 mt-4">
          ...
          {popularSearches.map((tag) => (
            <button
              ...
              className="px-3 py-1.5 rounded-full bg-surface-container text-on-surface-variant font-ui-caption text-ui-caption hover:bg-secondary-container hover:text-on-secondary-container transition-all duration-200 border border-outline-variant hover:border-secondary"
            >
              {tag}
            </button>
          ))}
